...
(defn change-batch-quantity [command repo]
  (let [product (repository/get-by-ref repo (:ref command))]
    (let [p (product/change_batch_quantity product command)]
      (repository/save repo p)
      (run! message-bus/emit! (:events p))
      )
    ))

(defn reallocate [event repo]
  (dispatch! (commands/allocate event) :repo repo))

(defn send-out-of-stock-notification [event]
  (println (str "Out of stock for 12 " (:sku event))))

(def start (fn [repo]
             (message-bus/subscribe! :out-of-stock send-out-of-stock-notification)
